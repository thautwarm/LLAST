...
let rec emit (types: type_table) (proc: ref<proc>) =
    ...
    let rec emit' (ctx: context): (llvm -> symbol)  =
        ...
        | Conv conversion ->
            ...
            | PendingConv _ -> failwith "Undecided conversion."
            | ZeroExt(src, dest) ->
        ...
        | Acc acc ->
        match acc with
        | PendingAcc _ -> failwith "Undecided data accessing."
        | Alloca(ty, Some(data)) ->
